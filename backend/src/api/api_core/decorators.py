        ...
        def decorator(*args, **kwargs):
            ...
            if allowed_roles:  # if something is passed, check permissions
                ...
                if user_role is None:
                    raise Unauthorized(
                        "User role undefined!"
                    )  # something wrong for additional_claims in creating access token
                    
                if user_role not in allowed_roles:
                    raise Forbidden(
                        "Insufficient permissions to access route"
                    )  # has valid token, not right roles
...
def register_jwt_access_token_refresh(app):
    ...
    def refresh_expiring_jwts(response):
        try:
            ...
            ):  # doesn't issue a token refresh when someone logs out
                return response
            ...
            if target_timestamp > exp_timestamp:
                additional_claims = {
                    "user_role": user_role
                }  # preserve role when refreshing access token
